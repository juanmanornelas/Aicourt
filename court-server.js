...
function purgeOldCases() {
  ...
  for (const id of Object.keys(store)) {
    const filed = new Date(store[id].filedAt).getTime();
    if (now - filed > THREE_MONTHS_MS) {
      delete store[id];
      removed++;
    }
  }
  if (removed > 0) {
    writeStore(store);
    console.log(`[cleanup] Purged ${removed} old case(s)`);
  }
}
...
app.get('/', (req, res) => {
  const store = readStore();
  res.json({ status: 'Pettite Court API is in session', cases: Object.keys(store).length });
});
...
app.post('/case', (req, res) => {
  try {
    ...
    writeStore(store);
    console.log(`[${new Date().toLocaleTimeString()}] CASE FILED: ${id.toUpperCase()}`);
    res.json({ ok: true });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
...
app.listen(PORT, () => {
  ...
  console.log(`Persistent: ${fs.existsSync('/data') ? 'YES' : 'NO — add a volume on Railway'}`);
});
