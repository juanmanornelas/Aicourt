...
// ── RAILWAY API ───────────────────────────────────────────────────────────
const API = 'https://aicourt-production.up.railway.app'
async function storageSet(id, data) {
  const res = await fetch(`${API}/case`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, data }) })
...
export default function PettitCourt(){
  ...
  const fileCase=async()=>{
    ...
    try{await storageSet(id, caseData);setCaseId(id);setPhase("summoned");}
  ...
  const lookupCase=async()=>{
    ...
    try{
      ...
      if(!data){setError("Case not found. Check your code and try again.");setLookupLoading(false);return;}
    ...
    try{
      const res=await fetch(`${API}/verdict`,{
        ...
        body:JSON.stringify({
          caseId,
          userText,
          plaintiffImgs:(loadedCase?.plaintiffImgs||[]),
          defImgs:await Promise.all(defImgs.map(async f=>({type:"image/jpeg",data:await compress(f)})))
        })
      });
      const data=await res.json();
      if(!data.ok) throw new Error("Verdict failed");
      setVerdict(data.verdict);setPhase("verdict");
  ...
  const Header=()=>(
    <div style={{borderBottom:"1px solid rgba(200,168,75,0.2)",textAlign:"center",padding:"24px 20px 20px",background:"rgba(200,168,75,0.03)",cursor:"pointer"}} onClick={reset}>
      ...
      <div style={{fontFamily:"'Cinzel Decorative','Cinzel',serif",fontSize:"clamp(20px,5vw,34px)",fontWeight:900,letterSpacing:"0.1em",color:"#DDD0B0",marginBottom:"3px"}}>PETTITE COURT</div>
      ...
      <div style={{maxWidth:"700px",margin:"0 auto",padding:"36px 20px 60px"}}>
        ...
        {phase==="summoned"&&<div className="fadeUp">
          ...
          <div style={{background:"rgba(200,168,75,.08)",border:"1px solid rgba(200,168,75,.3)",borderRadius:"6px",padding:"22px",textAlign:"center",marginBottom:"18px"}}>
            ...
            <div style={{fontSize:"12px",fontStyle:"italic",color:"rgba(200,168,75,.4)",marginTop:"8px"}}>Defendant enters this at pettitecourt.com</div>
          </div>
